...
const Login: React.FC<LoginProps> = ({ onLogin }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // Call user's local backend
      const response = await apiService.auth.login({ email, password });
      // If success, update state and navigate
      onLogin();
      navigate('/dashboard');
    } catch (err: any) {
      ...
      <div className="flex-1 flex items-center justify-center p-8 bg-white lg:bg-transparent">
        <div className="w-full max-w-lg bg-white rounded-[24px] p-12 shadow-[0_20px_60px_rgba(0,0,0,0.05)] border border-gray-100">
          <div className="flex bg-gray-50 p-1.5 rounded-2xl mb-12">
            <button className="flex-1 text-center py-2.5 text-sm font-bold bg-white rounded-xl shadow-sm text-gray-800">Sign in</button>
            <Link to="/register" className="flex-1 text-center py-2.5 text-sm font-medium text-gray-400 hover:text-gray-600 transition-all">Sign up</Link>
          </div>

          <div className="space-y-2 mb-10">
            <h2 className="text-3xl font-bold text-gray-900">Welcome back!</h2>
            <p className="text-gray-400 font-medium">Sign in with your Avagama.ai account</p>
          </div>
            ...
            <div className="space-y-2">
              ...
              <div className="relative">
                <input 
                  ...
                  required
                />
                <button type="button" className="absolute right-5 top-1/2 -translate-y-1/2 text-gray-400">
                  <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z" /><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z" /></svg>
                </button>
              </div>
              <div className="text-right">
                <button type="button" className="text-sm text-[#a26da8] font-medium hover:underline">Forget Password?</button>
              </div>
            </div>
