...
const EvaluateProcess: React.FC = () => {
  ...
  if (isEvaluating) {
    return (
      <div className="fixed inset-0 bg-white z-[100] flex flex-col items-center justify-center p-12 overflow-hidden">
        <div className="relative w-[600px] h-[600px] flex items-center justify-center">
          <div className="text-center z-10 animate-pulse">
             <h2 className="text-2xl font-black text-gray-800 tracking-tight">Evaluating</h2>
             <p className="text-xl font-bold text-gray-400">your process..</p>
          </div>
          {dimensions.map((dim, i) => {
            ...
            return (
              <div 
                key={i}
                className="absolute transition-all duration-500 flex flex-col items-center gap-3"
                style={{ 
              ...
              >
                <div className={`w-16 h-16 rounded-3xl border-2 flex items-center justify-center shadow-lg transition-all ${dim.color} ${isActive ? 'shadow-xl -translate-y-2' : ''}`}>
                  <svg className="w-8 h-8" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d={dim.icon} />
                  </svg>
                </div>
                <p className={`text-[10px] font-black uppercase tracking-tighter text-center max-w-[100px] leading-tight transition-colors ${isActive ? 'text-gray-900' : 'text-gray-400'}`}>
                  {dim.name}
                </p>
              </div>
            );
  ...
  return (
    <div className="p-8 space-y-8 bg-gray-50 min-h-screen relative">
      <div className="flex justify-between items-center">
        ...
        <div className="flex gap-4">
          ...
          <button
            ...
            className="bg-[#9d7bb0] text-white px-8 py-2 rounded-lg font-bold flex items-center gap-2 hover:bg-[#8b6aa1] transition-all shadow-lg shadow-purple-100"
          >
      ...
      <div className="bg-white rounded-[40px] border border-gray-100 shadow-sm p-12 max-w-5xl mx-auto min-h-[650px] flex flex-col">
        ...
        <div className="relative flex justify-between items-start mb-24 px-10">
          {/* Background Connecting Line */}
          <div className="absolute top-5 left-[calc(40px+2.5rem)] right-[calc(40px+2.5rem)] h-[2px] bg-gray-100 z-0">
            <div 
          ...
          {stepsList.map((s) => (
            <div key={s.id} className="relative z-10 flex flex-col items-center gap-3">
              <div className={`w-10 h-10 rounded-full flex items-center justify-center font-bold text-sm transition-all duration-500 ring-4 ring-white ${step >= s.id ? "bg-[#9d7bb0] text-white shadow-lg" : "bg-gray-100 text-gray-400"}`}>
                {s.id}
              </div>
              <span className={`absolute top-12 whitespace-nowrap text-[10px] font-bold uppercase tracking-widest text-center ${step >= s.id ? "text-[#9d7bb0]" : "text-gray-400"}`}>
                {s.label}
              </span>
            </div>
          ))}
          ...
          {step === 2 && (
            <div className="space-y-10 animate-fadeIn">
              <div className="max-w-xl mx-auto space-y-8 text-center">
                <div className="space-y-2">
                  <h3 className="text-2xl font-black text-gray-900 tracking-tight">Upload SOP Document</h3>
                ...
                <div 
                  ...
                  className={`relative group cursor-pointer p-14 border-2 border-dashed rounded-[48px] transition-all flex flex-col items-center justify-center gap-6 ${selectedFile ? 'border-[#4db6ac] bg-[#4db6ac]/5' : 'border-gray-100 hover:border-[#9d7bb0]/30 hover:bg-gray-50'}`}
                >
                  <input 
                    type="file" 
                    ref={fileInputRef}
                  ...
                  <div className={`w-24 h-24 rounded-[32px] flex items-center justify-center text-4xl transition-transform group-hover:scale-110 ${selectedFile ? 'bg-white text-[#4db6ac] shadow-lg shadow-teal-100' : 'bg-white text-[#9d7bb0] shadow-lg shadow-purple-100'}`}>
                    {selectedFile ? '📄' : '📤'}
                  </div>

                  <div className="space-y-2">
                    <p className={`text-base font-black uppercase tracking-widest ${selectedFile ? 'text-[#4db6ac]' : 'text-gray-900'}`}>
                  ...
                  {selectedFile && (
                    <button 
                      ...
                      className="absolute top-6 right-6 p-2.5 bg-white rounded-full text-red-400 hover:text-red-600 shadow-sm border border-gray-100"
                    >
                ...
                <div className="p-6 bg-blue-50 rounded-3xl border border-blue-100 flex items-start gap-4 text-left">
                  ...
                  <div className="space-y-1">
                    <p className="text-[10px] font-black text-blue-900 uppercase tracking-[0.2em]">Contextual Grounding</p>
                    <p className="text-xs text-blue-700 leading-relaxed font-medium">Documents are processed using secure ephemeral embeddings to provide evidence-based scoring recommendations.</p>
                  </div>
                </div>
              </div>
            </div>
          ...
          {step === 3 && (
            <div className="space-y-10 animate-fadeIn">
              <div className="grid grid-cols-2 gap-8">
                <div className="space-y-2">
                  <label className="text-xs font-black text-gray-400 uppercase tracking-widest ml-2">Volume (tx / Month)</label>
                  <input
                    name="volume"
                ...
                <div className="space-y-2">
                  <label className="text-xs font-black text-gray-400 uppercase tracking-widest ml-2">Frequency</label>
                  <select
                    name="frequency"
            ...
            <div className="space-y-8 animate-fadeIn pb-10">
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                ...
                <div className="bg-gray-50 p-10 rounded-[40px] border border-gray-100 space-y-6">
                  <h3 className="text-xs font-black text-gray-400 uppercase tracking-[0.2em] border-b border-gray-200 pb-4">Process Identity</h3>
                  <div className="space-y-4">
                    <div className="flex flex-col">
                      <span className="text-[10px] font-black text-gray-400 uppercase">Process Name</span>
                      <span className="text-base font-bold text-gray-900">{formData.processName || "Unnamed Process"}</span>
                    </div>
                    <div className="flex flex-col">
                      <span className="text-[10px] font-black text-gray-400 uppercase">Assigned Agent</span>
                      <span className="text-sm font-bold text-[#9d7bb0]">{formData.agentType}</span>
                    </div>
                    <div className="flex flex-col">
                      <span className="text-[10px] font-black text-gray-400 uppercase">Ingested Documentation</span>
                      <span className={`text-sm font-bold flex items-center gap-2 ${selectedFile ? 'text-[#4db6ac]' : 'text-gray-400'}`}>
                        {selectedFile ? (
                          <>
                ...
                {/* Operations Section */}
                <div className="bg-gray-50 p-10 rounded-[40px] border border-gray-100 space-y-6">
                  <h3 className="text-xs font-black text-gray-400 uppercase tracking-[0.2em] border-b border-gray-200 pb-4">Operational Metrics</h3>
                  <div className="grid grid-cols-2 gap-x-6 gap-y-4">
                    <div className="flex flex-col">
                      <span className="text-[10px] font-black text-gray-400 uppercase">Monthly Volume</span>
                      <span className="text-sm font-bold text-gray-900">{formData.volume} tx</span>
                    </div>
                    <div className="flex flex-col">
                      <span className="text-[10px] font-black text-gray-400 uppercase">Frequency</span>
                      <span className="text-sm font-bold text-gray-900">{formData.frequency}</span>
                    </div>
                    <div className="flex flex-col">
                      <span className="text-[10px] font-black text-gray-400 uppercase">Complexity</span>
                      <span className="text-sm font-bold text-gray-900">{formData.complexity} / 10</span>
                    </div>
                    <div className="flex flex-col">
                      <span className="text-[10px] font-black text-gray-400 uppercase">Exception Rate</span>
                      <span className="text-sm font-bold text-[#4db6ac]">{formData.exceptionRate}%</span>
                    </div>
                  </div>
                </div>

                {/* AI Configuration Section */}
                <div className="bg-gray-50 p-10 rounded-[40px] border border-gray-100 space-y-6 lg:col-span-2">
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
                    <div className="space-y-6">
                      <h3 className="text-xs font-black text-gray-400 uppercase tracking-[0.2em] border-b border-gray-200 pb-4">Intelligence Stack</h3>
                      <div className="space-y-4">
                        <div className="flex flex-col">
                          <span className="text-[10px] font-black text-gray-400 uppercase">Synthesizer Engine</span>
                          <span className="text-sm font-bold text-[#9d7bb0]">{formData.model.split('-').map(w => w.charAt(0).toUpperCase() + w.slice(1)).join(' ')}</span>
                        </div>
                        <div className="flex flex-col">
                          <span className="text-[10px] font-black text-gray-400 uppercase">Risk Appetite</span>
                          <span className="text-sm font-bold text-gray-900">{formData.riskTolerance}</span>
                        </div>
                      </div>
                    </div>
                    <div className="space-y-6">
                      <h3 className="text-xs font-black text-gray-400 uppercase tracking-[0.2em] border-b border-gray-200 pb-4">Strategic Context</h3>
                      <div className="flex flex-col">
                        <span className="text-[10px] font-black text-gray-400 uppercase">Description</span>
                        <span className="text-xs font-medium text-gray-600 leading-relaxed italic">
      ...
      {showConfirm && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-md z-[110] flex items-center justify-center p-6">
          <div className="bg-white rounded-[50px] p-16 max-w-xl w-full shadow-2xl space-y-10 border border-white/20 animate-scaleUp">
            <div className="w-24 h-24 bg-[#9d7bb0]/10 text-[#9d7bb0] rounded-[30px] flex items-center justify-center text-4xl mx-auto border border-[#9d7bb0]/20">🤖</div>
            <div className="text-center space-y-4">
              <h3 className="text-3xl font-black text-gray-900 tracking-tight">Initiate AI Assessment?</h3>
              <p className="text-gray-500 text-lg leading-relaxed">
                Trigger the <strong className="text-gray-900">{formData.model}</strong> Agent to perform deep 10-dimensional analysis {selectedFile && 'using your SOP reference'}.
              </p>
            </div>
            <div className="flex gap-4">
